...
func (in *CPUProvisionConfig) DeepCopyInto(out *CPUProvisionConfig) {
	*out = *in
	if in.IndicatorTargets != nil {
		in, out := &in.IndicatorTargets, &out.IndicatorTargets
		*out = make(map[QoSRegionType][]IndicatorTargetConfiguration, len(*in))
		for key, val := range *in {
			var outVal []IndicatorTargetConfiguration
			if val == nil {
				(*out)[key] = nil
			} else {
				in, out := &val, &outVal
